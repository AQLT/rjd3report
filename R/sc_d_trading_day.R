trading_day_pattern <- function(x){
  data <- rjd3toolkit::user_defined(x, c("regression.ntd", "tde", "y", "log", "tde_f"))
  is_multiplicative <- as.logical(data[["log"]])
  ntd <- data[["regression.ntd"]]
  freq <- frequency(data[["tde"]])
  ...
  if(ntd == 0){
    ...
    data_td <- do.call(
      rbind,
      rjd3toolkit::user_defined(x, sprintf("preprocessing.model.td(%s)", seq_len(ntd)))
    )
    regression_var_names <- rownames(data_td)
    estimated_values <- data_td[,1]*100
    evolution <- c(tail(data[["tde"]], 2),
                   head(data[["tde_f"]],1))
